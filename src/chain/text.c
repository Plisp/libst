 ...
 *      \-+ <-- +-----+ <-- +---------------+ <-- +-/
 */
bool st_insert(Text *txt, size_t pos, const char *data, size_t len) {
	if (len == 0)
		return true;
	if (pos > txt->size)
		return false;
 ...
 *      \-+ <-- +----+ <-- +--+ <-- +-/
 */
bool st_delete(Text *txt, size_t pos, size_t len) {
	if (len == 0)
		return true;
	size_t pos_end;
...
bool text_delete_range(Text *txt, const Filerange *r) {
	if (!text_range_valid(r))
		return false;
	return st_delete(txt, r->start, text_range_size(r));
}
...
SliceIter *st_iter_to(SliceIter *it, size_t pos) {
	text_iterator_init(it->piece->text, it, pos);
	return it;
}

SliceIter *st_iter_new(SliceTable *txt, size_t pos) {
	SliceIter *it = malloc(sizeof *it);
	text_iterator_init(txt, it, pos);
	return it;
}
void st_iter_free(Iterator *it) { free(it); }
...
void st_print_struct_sizes(void) {
	fprintf(stderr,
		"Implementation: \e[38;5;1mlinked list piece table\e[0m\n"
		"sizeof(struct Piece): %zd\n"
		"sizeof(SliceTable): %zd\n",
		sizeof(struct Piece), sizeof(SliceTable));
}

size_t st_size(const Text *txt) {
	size_t size = 0;
	for (const Piece *p = &txt->begin; p->next; p = p->next) {
		size += p->len;
	}
	return size;
}

int st_depth(const Text *txt) {
	int count = 0;
	for (const Piece *p = &txt->begin; p->next; p = p->next)
		count++;
	return count;
